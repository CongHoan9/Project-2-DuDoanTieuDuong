...
(() => {
    ...
    async function init() {
        ...
        state.client.auth.onAuthStateChange((_, session) => {
            ...
            if (session) {
                // Cập nhật giao diện (nav bar) NGAY LẬP TỨC để hết cảm giác "khựng"
                renderNav();
                
                // Sau đó mới tải dữ liệu ngầm
                loadProfile().then(() => {
                    renderNav(); // Cập nhật lại nav nếu full_name có thay đổi
                    fillAccount();
        ...
        document.addEventListener("submit", async e => {
            ...
            if (form.id === "authForm") {
                ...
                try {
                    ...
                    if (state.session) {
                        // Đóng modal và báo thành công ngay lập tức (không block UI)
                        closeModal("authModal"); 
                        toast(mode === "signup" ? "Đăng ký thành công" : "Đăng nhập thành công");
                        
                        // Xử lý kéo data chạy ngầm (background)
                        (async () => {
                            await ensureMyProfile(); 
                            await loadProfile().catch(x => toast(x.message));
